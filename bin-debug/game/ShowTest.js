...
var TestA = (function () {
    function TestA(m) {
        ...
        this._main.addEventListener(egret.TouchEvent.TOUCH_BEGIN, function (evt) {
    ...
    p.launchAnimations = function () {
        ...
        this._nScaleBase = 0;
        this._main.addEventListener(egret.Event.ENTER_FRAME, function (evt) {
            switch (_this._iAnimMode) {
                case TestA.ANIM_ROT:
                    _this.bird.rotation += TestA.STEP_ROT;
                    break;
                case TestA.ANIM_SCALE:
                    _this.bird.scaleX = _this.bird.scaleY = 0.5 + 0.5 *
                        Math.abs(Math.sin(_this._nScaleBase += TestA.STEP_SCALE));
                    break;
            }
            return false;
        }, this);
